...
const getCategoryImage = (category: string, modelName: string, index: number = 1): string => {
  ...
  switch(folderName) {
    case 'Power&VolumeButtons':
      ...
      return `${basePath}/Power&VolumeButtons${index}${index === 1 || index === 5 ? '.png' : '.jpeg'}`;
    case 'Cameras&Lens':
      return `${basePath}/cameras&lens${index}.jpeg`;
    case 'ChargingPorts&cables':
      return `${basePath}/ChargingPorts&cables${index}.jpeg`;
    case 'Screens&Displays':
      return `${basePath}/Screens&Displays${index}.jpeg`;
    case 'Speakers&AudioParts':
      return `${basePath}/Speakers&AudioParts${index}.jpeg`;
    case 'batteries':
    default:
      return `${basePath}/battery${index}.jpeg`;
  }
};
